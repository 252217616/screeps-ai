...
const SpawnExtension = {
    ...
    work() {
        if (this.spawning) {
            ...
            if((!global.taskRecord.get(this.room.name).has(this.id) || global.taskRecord.get(this.room.name).get(this.id)) && this.store[RESOURCE_ENERGY] != 300){
                    ...
                    global.taskRecord.get(this.room.name).set(this.id,true);//防止重复发布任务
                }
                // if (
    ...
    mySpawnCreep(configName) {
        ...
        // 获取身体部件, 优先使用 bodys
        const bodys = (typeof creepWork.bodys === 'string') ? this.getBodys(creepConfig.bodys ) : creepConfig.bodys as BodyPartConstant[]
        if (bodys.length <= 0) return ERR_NOT_ENOUGH_ENERGY
        
        const spawnResult = this.spawnCreep(bodys, configName, {
            memory: creepMemory
        })
        // 检查是否生成成功
        if (spawnResult == OK) {
